    ...
    function execAuth(resultCode, errMsg) {
        if (resultCode != "Success") {
            ...
            ThreeDSAdapter.authenticate({ 
                ...
                tx: 0,        
                sf: 0,
                em: 'sample@sample.com', // 請求先に紐づく有効状態の先頭部署に登録されているメールアドレスと同じ値での設定が必須となります。
                pn: '0300000000', // 請求先に紐づく有効状態の先頭部署に登録されている電話番号と同じ値での設定が必須となります。
...
#### 登録時の電話番号およびメールアドレスのパラメータに関する注意点

- 現状のシステム仕様では、決済時に各請求先に紐づいている有効状態の先頭部署に設定した電話番号およびメールアドレスが使用されます。
- この仕様により、3Dセキュア2.0の認証時に指定する em（メールアドレス）および pn（電話番号）のパラメータの値が有効状態の先頭部署に登録された電話番号およびメールアドレスと異なる場合、決済システム側で認証情報の不一致となり処理エラー（ER059、ER060など）が発生するためご注意ください。
- クレジットカード登録フローとして、3Dセキュア2.0の認証前に請求先部署の電話番号およびメールアドレスの登録を行う必要があります。
