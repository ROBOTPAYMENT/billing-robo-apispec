...
## リクエスト
- Path: `/api/v1.0/billing/bulk_stop`
...
| del_flg                                  | 請求先削除フラグ <br> 0:停止 <br> 1:削除 <br> ※請求先部署のみまたは請求先決済手段のみを削除する時は、省略 | 1    | 数値                                   |      |
...
| del_flg | 請求先部署削除フラグ <br> 0:停止 <br> 1:削除  | 1    | 数値                                   | (請求先部署削除時)                          |
...
| del_flg | 決済情報削除フラグ <br> 0:停止 <br> 1:削除 | 1    | 数値                                   | (請求先決済手段削除時) |
...
| 1212         | 請求先部署番号と請求先部署コードは同時に指定できません   |
...
| 1227         | まるなげ与信が申請中又は有効のため請求先の停止に失敗    |
| 1228         | まるなげ与信が申請中又は有効のため請求先の削除に失敗    |
