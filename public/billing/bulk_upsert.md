...
| cc_email                                                                          | CC送信先メールアドレス <br> ※両端のスペース除去 <br> ※追加省略時、nullで登録される                                                                                                                                                          | 256  | [メール形式](../../index.md#種別)      |                                      |
...
| customer_number            | 顧客番号 <br> ※payment_method=5の場合に必要(入力は任意) <br> ※追加省略時、nullで登録される                                                                                                                                                                       | 20       | [半角英数](../../index.md#種別)        |                                    |
| bank_code                  | 銀行コード <br> ※payment_method=3,4,5,9の場合に必要(入力は任意) <br> ※追加省略時、空で登録される                                                                                                                                                                 | 4        | 数値                                   |                                    |
| bank_name                  | 銀行名 <br> ※payment_method=5の場合に必要(入力は任意) <br> ※ゆうちょの場合は「ﾕｳﾁﾖ」 <br> ※追加省略時、nullで登録される                                                                                                                                          | 15       | [銀行名等](../../index.md#種別)        |                                    |
| branch_code                | 支店コード <br> ※payment_method=3,4,5,9の場合に必要(入力は任意) <br> ※追加省略時、空で登録される <br> ※payment_method=3でゆうちょの時の桁数は5桁                                                                                                                 | 3または5 | 数値                                   |                                    |
| branch_name                | 支店名 <br> ※payment_method=5の場合に必要(入力は任意) <br> ※追加省略時、nullで登録される                                                                                                                                                                         | 15       | [銀行名等](../../index.md#種別)        |                                    |
...
| bank_account_name          | 口座名義 <br> ※payment_method=3,4,5の場合に必要(入力は任意) <br> ※payment_method=3の場合の桁数は30桁 <br> ※payment_method=3の場合は半角スペース入力不可 <br> ※追加省略時、空で登録される                                                                         | 30       | [口座名義](../../index.md#種別)        |                                    |
...
| 1174         | 請求先部署売掛金補助科目コードが不正 v1.13.6                                         |
| 1175         | 請求先部署前受金補助科目コードが不正 v1.13.6                                         |
| 1176         | 請求先部署仮受金補助科目コードが不正 v1.13.6                                         |
| 1177         | 合計請求書の設定で登録済みの請求情報があります。                                     |
| 1178         | クレジットカード情報登録メール送信に失敗しました。                                   |
| 1179         | 手数料自動消込許容金額が不正 v1.13.10                                                |
| 1180         | バーチャル口座割当てに失敗 v1.19.0                                                   |
| 1181         | バーチャル口座が使用中 v1.19.0                                                       |
| 1182         | 利用可能なバーチャル口座番号がない v1.19.0                                           |
| 1183         | クレジットカード登録メール送信先が不正 MIKATA-5138                                   |
