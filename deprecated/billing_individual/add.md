...
## リクエスト
- Path: `/api/billing_individual/add`
...
| 1038         | 請求先が存在しない                                                        |
| 1039         | 請求先部署コードが不正                                                    |
| 1040         | 合計請求書用請求先コードが不正                                             |
| 1041         | 合計請求書用請求先部署番号が不正                                           |
| 1042         | 合計請求書用請求先部署コードが不正                                         |
| 1043         | 請求書テンプレートコードが不正                                             |
| 1044         | 親請求先部署が存在しない                                                   |
| 1045         | 既に他の親請求先部署が存在する親請求先部署を指定しています                    |
| 1046         | 請求先部署の登録ステータスが不正                                            |
| 1049         | 合計請求書用請求先部署番号、合計請求書用請求先部署コードは同時に指定できません |
| 1050         | 親請求先部署は承認依頼中のため使用できません                                 |
| 1051         | 請求先決済情報コードが不正                                                  |
| 1052         | 請求先決済情報名が不正                                                      |
| 1053         | 請求先部署売掛金補助科目コードが不正                                         |
| 1054         | 請求先部署前受金補助科目コードが不正                                         |
| 1055         | 請求先部署仮受金補助科目コードが不正                                         |
